...
stored_etas: Dict[int, list] = {ev_id: [] for ev_id, ev in fleet.vehicles.items()}
...
# It is good practice to initialize
bestOfAll = None
#bestOfAll = [18, 15, 10, 9, 14, 17, 19, 20, 16, 4, 1, 3, 2, 6, 7, 5, 8, 13, 12, 11, '|',
#             -1, 21, 10.03621240411417, -1, 21, 3.3093834433210967, -1, 21, 11.178597501344296, 629.6532388963317] # 0-100
...
while not degraded:
    ...
    for ev_id, ev in fleet.vehicles.items():
        stored_etas[ev_id].append(ev.eta)
...
df_etas = pd.DataFrame(stored_etas)

df_eta.to_csv(f'{opt_folder}eta.csv')
df_etas.to_csv(f'{opt_folder}etas.csv')
