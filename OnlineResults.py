...
# Useful functions
def single_folder_data(folder: Path, source_folder: Path, fleet: Fleet.Fleet = None):
    ...
    constraints_data = single_folder_constraints(folder)
...
def folder_data(simulations_folder: Path, source_folder: Path, simulations_number=30,
    ...
    for simulation_folder in simulations_folder.iterdir():
        if simulation_folder.is_file():
            continue
        costs_row, constraints_row = single_folder_data(simulation_folder, source_folder, None)
...
def single_folder_constraints(folder: Path):
